...
import os
import pandas as pd
import serial  # シリアル通信用モジュールをインポート
...
class DataGraphApp(QMainWindow):
    def __init__(self):
        ...
        shutter_open_button = QPushButton("Shutter Open")
        shutter_open_button.clicked.connect(self.shutter_open)  # メソッドを定義
        shutter_layout.addWidget(shutter_open_button)

        shutter_close_button = QPushButton("Shutter Close")
        shutter_close_button.clicked.connect(self.shutter_close)  # メソッドを定義
        shutter_layout.addWidget(shutter_close_button)

        main_layout.addLayout(shutter_layout)  # メインレイアウトに追加
