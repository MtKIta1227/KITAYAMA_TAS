...
class DataGraphApp(QMainWindow):
    ...
    def plot_graph(self):
        ...
        plt.plot(x_dark_ref, results['Difference_ref'], color='r', alpha=0.6, linestyle='-', label='a = Difference_ref')
        plt.plot(x_dark_ref, results['Difference_sig'], color='b', alpha=0.6, linestyle='-', label='b = Difference_sig')
        plt.plot(x_dark_ref, results['Difference'], color='k', alpha=0.9, linestyle='-', label='b-a')
    ...
    def save_data_to_excel(self):
        ...
        #パルスが設定されていない場合はシートをdefaultとする
        if not self.pulse_data:
            self.pulse_data['default'] = {label: self.text_boxes[label].toPlainText() for label in self.text_boxes}
